...
%Generally interesting in forecasting. and always looking for new collaborations. insights in forecasting. active sampling to boost predictive performance. 
...
\end{align}
where $\bSigma_X = \Cov(\X)$ and $\diag(\bSigma_X)$ denotes the diagonal of $\bSigma_X$. This places specific constraints on the covariance among the forecasts. In particular, the matrix $\bSigma$ is positive semi-definite if and only if
% Intuitively, 
...
all the forecasts $X_j$ are consistent with some (possibly different) sets of information about a common $Y$. \cite{satopaa2015partial} discuss estimation under this constraint. 
