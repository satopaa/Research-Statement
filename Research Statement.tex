...
Assuming measurement error in prediction polls is not only intuitively implausible but 
...
%In particular,
simple measurement-error-based aggregates of probability forecasts, such as the average or the median, can be improved dramatically by \textit{extremizing}, that is, by shifting them systematically closer to their nearer extreme (at zero or one). These and many other studies on extremization (see, e.g., \citealt{Ranjan08, Erev1994,
shlomi2010subjective, karmarkar1978subjectively, mellers2014psychological})  represent the unwieldy position of the
...
Forecast aggregation literature by and large agrees that the goal is to collect and combine  information from different forecasters (see, e.g., \citealt{forlines2012heuristics, armstrong2, dawid1995coherent}). In fact, \cite{satopaamodeling} provide simple examples showing that optimal aggregation is not well-defined without assumptions on the information structure among the forecasters. To illustrate, consider two forecasters whose information sets are shown in Figure \ref{InfoStr}. In this diagram full information is represented by the unit interval. Forecaster 1 knows $U \cup M$ while forecaster 2 knows $M \cup V$. Therefore nobody knows information in $W$. Unfortunately, averaging aggregators assume that all forecasters use the same information and hence never appear more informed than the most informed individual forecaster \citep{satopaa2015combining}. This dramatically under-estimates the group's information, namely $U \cup M \cup V$, and consequently leads to an under confident aggregate. The partial information framework, on the other hand, assumes partially overlapping information sets and aims to incorporate all of the forecasters' information in the resulting aggregate.

\begin{figure}[t!]
   \centering
   \includegraphics[width = \textwidth]{N=2.png} % requires the graphicx package
  \caption{Illustration of Information Distribution among Two Forecasters. The bars leveled horizontally with Forecaster $j$ represent the $j$th forecaster's information set.}
     \label{InfoStr}
\end{figure}
...
In practice, however, all the forecasters' information is not available to the aggregator. Instead, the forecasters typically reflect their information only through the predictions. Therefore the relevant aggregator under each specific instance of the framework is the \textit{revealed} aggregator $X'' := \E (Y \|
...
The framework, as introduced above, is clearly too abstract to be applied in practice. For one, due to the inability of the forecasters to identify clearly the basis to their predictions, the details of the information sets can almost always be rendered unattainable. Fortunately, it is possible to deduce the amount of information used by a forecaster.  \cite{satopaa2015partial} show that, without any further assumptions, the covariance matrix $\bSigma_X$ of the $X_j$'s extends to the unknown $Y$ as follows:
\begin{align}
...
\end{align}
where $\diag(\bSigma_X)$ denotes the diagonal of $\bSigma_X$. They explain that this covariance matrix describes how information is distributed among the forecasters. In particular, $\Var(Y)$ is the maximum amount of information that can be known about $Y$, $\Var(X_j)$ is the amount of information used by forecaster $j$, and 
...
%represents the total amount of information and $\Var(X_j)$ is the amount of information used by forecaster $j$. 
 ...
 \end{align*}
 Therefore increased variance suggests more information and is generally deemed helpful. This stands in clear contrast with standard statistical models that often regard higher variance as increased noise.  Why does this work? First, note that a fully informed forecaster predicts  $X_j = Y$ while a forecaster with no information predicts the prior mean $X_j = \E(Y) = \E(Y|\F_0)$, where $\F_0 = \{\emptyset, \Omega\}$ is the trivial $\sigma$-field. Therefore as the forecaster becomes more informed, $\F_j$ grows from $\F_0$ to $\F$ and, consequently, $\Var(X_j)$ increases from $0$ to $\Var(Y)$. Second, two forecasters with high information overlap are likely to report very similar predictions and hence present higher covariance.

If the partial information framework is to ever become common practice in forecast aggregation, a close yet practical specification within the framework must be introduced. This requires one additional assumption, namely the choice of a joint distribution for the model variables $Y, X_1, \dots, X_N$.
...
%  The resulting model, known as the \textit{Gaussian partial information model}, turns out extremely successful. 
...
%  
They apply the resulting model, known as the \textit{Gaussian partial information model}, to real-world prediction polls, involving both probability and real-valued forecasts. In both cases, the revealed aggregator achieves a noticeable performance improvement over the common measurement-error-based aggregators, suggesting that  information diversity is the more important  source of forecast heterogeneity. 
