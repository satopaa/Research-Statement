...
%\subsection*{Motivation} \vspace{-0.2em}
 \vspace{0.5em}
\noindent
\textbf{2. MODELING, AGGREGATING, AND IMPROVING PREDICTIONS:} Past literature has distinguished two types of polling: prediction and opinion polling. In broad terms, an opinion poll is a survey of public opinion, whereas a prediction poll involves multiple agents collectively predicting the value of some quantity of interest \citep{goel2010prediction}. 
...
%This motivated
This prompted the initiation of a government funded research group called the Good Judgment Project (GJP) (\citealt{ungar2012good}). During its four years of operation, the GJP recruited thousands of forecasters who gave hundreds of thousands of probability estimates about hundreds of future events. Given that these events were chosen by the Intelligence Advanced Research Projects Activity (IARPA), one of the core research objectives was to investigate how prediction polls can improve national security decision-making. This task is non-trivial because typically it is not possible to determine ex-ante which forecaster will be the most accurate, and even if this could be done, heeding only the most accurate forecaster's advice would ignore a potentially large amount of relevant information that is being contributed by the rest of the forecasters. Therefore a better alternative is to combine the
...
Consequently, measurement-error-based aggregators, such as the average, median, and other measures of central tendency, are suboptimal. 
...
measures of central tendency fail to do so \citep{satopaa2015combining}. To illustrate, consider two forecasters, each of whom reports a probability estimate for the occurrence of some future event. Suppose that they have access to different information. More specifically, in reference to the diagram in Figure \ref{InfoStr}, 
...
their estimates are based on the partial information sets $U \cup M$ and $M \cup V$, respectively. Now, if both forecasters predict $0.9$, 
%
...
% 
 then clearly the combined information $U \cup M \cup V$ should give an aggregate forecast somewhat greater than $0.9$. In this simple scenario, however, measures of central tendency will always equal to $0.9$. Unfortunately, they fail to assume information heterogeneity among the forecasters and hence end up under-estimating the amount of combined information. As a result, they are under-confident and have poor out-of-sample performance  \citep{satopaa, satopaa2014probability}. 
...
Therefore, improving the general practice of aggregation is hindered by the lack of a formal description of a more appropriate source of forecast heterogeneity. 
...
In response to this need, for the past three years I have collaborated with the GJP and used their large forecasting database to test hypotheses and develop fundamental theory of an alternative source of forecast heterogeneity. In particular, my dissertation work formalizes \textit{information diversity} as a source of heterogeneity. 
...
This forms the basis of a novel modeling framework, called the \textit{partial information framework}, that is extremely general and can be applied to a broad class of prediction problems. This framework is not only more plausible on the micro-level
...
\begin{figure}[t!]
  ...
  \caption{Illustration of Information Distribution among Two Forecasters. The top bar represents all possible information that can be known about $Y$. The bar leveled horizontally with Forecaster $j$ represents the $j$th forecaster's information set.}
     \label{InfoStr}
\end{figure}
...
Consider $N$ forecasters and suppose forecaster $j$ predicts $X_j$ for some quantity of interest $Y$. The partial information framework assumes that the observables $Y$ and $X_j$ are measurable random variables under some common probability space $(\Omega, \F , \P)$. 
...
%More specifically,
forecaster $j$ predicts $X_j = \E(Y \| \F_j)$ based on a partial information set $\F_j \subseteq \F$. Therefore $\F_i \neq \F_j$ if $X_i \neq X_j$, and forecast heterogeneity stems purely from information diversity.  
...
Even though the partial information framework, as introduced above, is intuitively appealing and certainly an accurate description of the real world, it is clearly too abstract to be applied in practice.  If it is to ever become common practice in forecast aggregation, a close yet practical specification within the framework must be introduced and made accessible to practitioners. Fortunately, this only requires one additional assumption, namely the choice of a joint distribution for the model variables $Y, X_1, \dots, X_N$. 
...
%A better alternative was discussed by \cite{satopaamodeling}. They assume that $\bSigma_X$ is compound symmetric and then combine the forecasts with the revealed aggregator under the corresponding Gaussian partial information model. 
 Developing a partial information aggregator that can be applied to a single set of forecasts and has no dependency on parameters is certainly a future research direction with potential for high impact. One idea is to first derive the aggregator based on the Gaussian model and then integrate out any unknown parameters with respect to their posterior distribution. Currently, we have
...
%is hosted by the U.S. Department of Health \& Human Services (HHS) with the goal of helping patients to make informed decisions about health care.
 \vspace{0.5em}
\noindent
\textbf{3. PUBLIC REPORTING OF HOSPITAL MORTALITY RATES:} 
...
% \vspace{-1em} \subsection{Future Research} \vspace{-0.2em}
 \vspace{0.5em}
\noindent
\textbf{3.1. FUTURE RESEARCH:} The largely unprincipled modeling practice in public reporting offers many research opportunities. For instance, we have serious shortcomings in the models used for ranking liver transplantation centers.  Even though our work corrects some of the common misconceptions and lays down foundations upon which more principled public reporting can be developed, much work remains to be done. One direction is to repeat our study of AMI on heart failure and pneumonia, for which we already have close imitations of Medicare's data. 
%have data that closely imitate that of Medicare's. 
% In particular, 
%The following enumeration provides some future direction. 
%\begin{enumerate}[i)]
%\item 
Another interesting avenue is to explore interactions between hospital and patient characteristics. For instance, 
%Second, our current models mostly separate patient and hospital characteristics. However, 
letting the slope coefficients of the patient characteristics to change, say, linearly in the hospital characteristics, would help in explaining what kind of hospitals are good at treating what kind of patients. Such a model could form the basis of a new publicly available website that enables patients to type in their personal information and compare performance estimates that are personalized and hence more relevant to them. 
...
\textbf{4. CONCLUSION: OTHER RESEARCH DIRECTIONS: }
...
Also, given my background in computer science, I am broadly interested
