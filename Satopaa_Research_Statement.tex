...
%\subsection*{Motivation} \vspace{-0.2em}
 \vspace{0.5em}
\noindent
\textbf{2. MODELING AND AGGREGATING PREDICTIONS:}  Modern social and computer networks have popularized \textit{prediction polling} -- a type of polling in which multiple
...
%Given that these events were chosen by the Intelligence Advanced Research Projects Activity (IARPA), one of the core research objectives was to investigate how prediction polls can improve national security decision-making. This task is challenging because
 Typically it is not possible to determine ex-ante which forecaster will be the most accurate, and even if this could be done, heeding only the most accurate forecaster's advice would ignore a potentially large amount of information that is being contributed by the rest of the forecasters. Therefore a better alternative is to aggregate the
...
In particular, it is necessary to specify how the forecasts differ from the true value of the target quantity.
 For the past several decades, potentially due to the early forms of data collection, {measurement error} has been considered as the main source of heterogeneity. 
% This approach has become the standard and is often applied in practice even when data variation is dominated  by other causes.
%  besides error in measurement. 
% For instance, measurement error may be reasonable for modeling repeated estimates from a single instrument. However, it is unlikely to hold in prediction polling, where estimates arise from multiple, often widely different sources.
...
Unfortunately, measurement-error-based aggregators, such as the average, median, and other measures of central tendency, do not capture the forecasters' combined information. To illustrate, consider two forecasters who use disjoint information sets to 
%use independent sets of information to
 estimate the probability of some future event. 
%Their information layout is shown in :
%  the information that two forecasters use in their estimated the probability of some future event. As is illustrated in Figure \ref{InfoStr}, 
%They, however, arrived at these predictions based on different sets of information.
% In particular, the forecasters use some information sets $U$ and $M$. 
% , where both $U$ and $M$ support the occurrence of the event.
  Figure \ref{InfoStr} shows this layout and denotes the forecasters' information sets with $U$ and $M$. Now, suppose that both $U$ and $M$ are in favor of the event and that each forecaster predicts $0.9$. 
% Figure \ref{InfoStr} illustrates the information that each forecaster uses: 
% with disjoint sets of information as illustrated in Figure \ref{InfoStr}. 
%two forecasters who use independent sets of evidence to estimate the probability of some future event. 
%% to be $0.9$ 
...
% As a result, both forecasters predict $0.9$. 
% and both forecasters predict $0.9$, 
%
...
% 
Given that the combined information $U \cup M$ would provide even stronger evidence in favor of the event, the optimal aggregate of the forecasts should be somewhat greater than $0.9$. In this simple scenario, however, all measures of central tendency aggregate to $0.9$. Therefore they fail to account for the  information heterogeneity among the forecasters.
 Instead, they assume all forecasters to use the same information and explain any remaining variation with measurement error. 
% This unfortunate because these aggregators 
%and, as a result, do not reflect the forecasters' combined information. 
Of course, this is only one simple example. Unfortunately, however, the result is general: it holds for any number of univariate forecasts -- even when the predictions are not identical and the forecasters' information sets are not disjoint \citep{satopaa2015combining}.
...
%\marginpar{explain that this is positive information}
...
%Clearly, there is a need for a new source of forecast heterogeneity. In response, m

Motivated by these shortcomings, I have collaborated with a government funded research group called the Good Judgment Project (\citealt{ungar2012good}) and used their large forecasting database to understand the main drivers of forecast heterogeneity.
%develop a more appropriate source of heterogeneity. 
%evaluate the appropriateness of information diversity.
As a result, my dissertation proposes and formalizes \textit{information diversity} as an alternative to measurement error \citep{satopaamodeling2, satopaamodeling}.
%source of forecast heterogeneity. 
...
%This theory has been tested in close collaboration with 
 %for the past three years I have collaborated with a government funded research group called the Good Judgment Project (\citealt{ungar2012good}) and used their large forecasting database to develop a more appropriate source of forecast heterogeneity. In particular, my dissertation formalizes and proposes \textit{information diversity} as an alternative to measurement error.
...
than any other framework has been to date. 
The framework is not only extremely general but also provides aggregators that provably capture the forecasters' combined information.
%collect information from the forecasters.
%, and c.
%
...
\vspace{-1em}
  ...
  \caption{Information Distribution among the Two Forecasters.
...
Consider $N$ forecasters predicting the value of some quantity of interest $Y$. The partial information framework assumes that $Y$ is a measurable random variable under some probability space $(\Omega, \F , \P)$. 
Here the principal $\sigma$-field $\F$ represents all possible information about $Y$. 
...
%More specifically,
%The 
Forecaster $j$ then predicts $X_j = \E(Y \| \F_j)$, where $\F_j \subseteq \F$ contains the information used in the forecast. That is,  if the forecaster uses a simple rule, $\F_j$ may not be the full $\sigma$-field of information available to the forecaster but rather a smaller $\sigma$-field corresponding to the information actually used by the rule. Therefore $\F_i \neq \F_j$ if $X_i \neq X_j$, and forecast heterogeneity stems purely from information diversity.  
...
In practice, however, all the details of the forecasters' information are not available to the aggregator. Instead, information comes to the aggregator only through the forecasts.
%the forecasters provide information only through their predictions. 
%The optimal aggregator within the framework uses all of this information to predict the value of $Y$. 
Therefore, the relevant aggregator is the \textit{revealed aggregator} $X'' := \E (Y \|
\F'')$, where $\F'' := \sigma(X_1, \dots, X_N)$ is the $\sigma$-field generated (or revealed) by the
$X_j$'s. This aggregator captures the forecasters' combined information and also
% If $\E(Y^2) < \infty$, then among all variables measurable with respect to $\F''$, the revealed aggregator 
minimizes the expected quadratic loss among all possible aggregators.
% and hence is the relevant aggregator within each specific instance of the framework. 
% Therefore it should be considered as the target for all forecast aggregation practice.
% of forecast aggregation. 
%reasonable upper bound on aggregation efficiency. 
%\cite{satopaa2015combining} show that $X''$ never reduces to a non-trivial weighted average of the forecasts, suggesting that information diversity, as a source of forecast heterogeneity, is fundamentally different from measurement error. 
...
Even though the framework, as briefly described above, is intuitively appealing and useful for proving theorems, it is clearly too abstract to be applied in practice. If it is to ever become widely used in forecast aggregation, a close yet practical specification within the framework must be introduced and made accessible to practitioners. Fortunately,  \cite{satopaamodeling2} show that the general framework poses a specific structure on the first two joint moments of $Y, X_1, \dots, X_N$. Thus, a specification only requires one additional assumption, namely the choice of a parametric family for the joint distribution of these variables. 
...
% In order to construct a more specific model within the framework, a joint distribution for the model variables $Y, X_1, \dots, X_N$ must be specified.
  \cite{satopaamodeling2} provide both intuitive and technical motivations for using the multivariate Gaussian distribution
...
This work proposes an alternative to a core statistical concept, namely measurement error, and therefore
...
\itemsep-0.3em 
\item
% \cite{satopaa2015partial} apply 
So far, the Gaussian model has been applied to forecasters predicting multiple related outcomes. 
% In the most extreme case, however, only a set of forecasts of a single unknown outcome are available. How should the forecasts be aggregated in such a low-data setting?
% \cite{satopaa2015combining} suggest that any type of weighted average (or some other measure of central tendency) is a poor choice. 
%A better alternative was discussed by \cite{satopaamodeling}. They assume that $\bSigma_X$ is compound symmetric and then combine the forecasts with the revealed aggregator under the corresponding Gaussian partial information model. 
 Therefore a natural next step is to develop an aggregator for a set of forecasts of a single outcome.
%  is a natural future direction.
%  with potential for high impact. 
  One idea is to first derive the aggregator based on the Gaussian model and then integrate out any unknown parameters with respect to their posterior distribution. Currently, we have
work in progress analyzing such \textit{Bayes-Gaussian aggregators} but much more work is needed \citep{ernst2015, satopaaBayesGaussian}. 
 
% \marginpar{Mention that previously used multiple problems...}
...
\item Information diversity is not only relevant in statistical estimation but also offers new directions in other areas of statistics. For instance, some of our current work applies information diversity to testing hypotheses about a binary outcome \citep{satopaaTesting}. Such procedures can be expected to yield higher power when information diversity is the dominant source of data variation. 
%Eventually we plan to extend this work to simultaneous testing of multiple informationally overlapping hypotheses. 

\item 
Most aggregation procedures use one estimate per forecaster, even though it is common for experts to update their beliefs over time. 
Motivated by this, \cite{satopaa2014probability} consider a dynamic context in which experts can update their beliefs at random intervals. Overall, this paper was very well received. The aggregator therein, however, is an empirical procedure that requires a training set of multiple prediction problems with known outcomes. Therefore, an interesting future project is to use martingale theory within the partial information framework and construct a time-series aggregator that can operate directly on the forecasts. 
%Recently, I was approached by a private sector party who expressed high interest in 
%\marginpar{This actually aims to estimate X'' via extremization. The problem is that it needs multiple problems with known outcomes. We want to have this for a single time-series setup.}
%\marginpar{bring back our old paper; need motivation for this problem.}


\item Information diversity can be also applied to model predictions. This suggests many contributions in machine learning. For instance, semi-supervised learning refers to setups with a large amount of data of which only a relatively few have been labeled. While the commonly used ensemble techniques, such as stacking or Bayesian averaging, require labeled observations to combine the individual models' predictions, partial information aggregators do not. This suggests an opportunity for improved aggregation efficiency and hence improved prediction accuracy.
...
Furthermore, our AMI mortality data, which are a very careful imitation of Medicare's data and consist of about 340,000 patients distributed across about 4,300 hospitals, support all of our models much more than the Hospital Compare model.
...
models would help us identify the types of patients that each hospital treats particularly well
%For instance, 
...
are facing in todayÕs Òbig dataÓ boom. All in all,  I enjoy working with subject-matter experts, always
