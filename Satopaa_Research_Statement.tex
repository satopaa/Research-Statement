...
measurement-error-based aggregators, such as the average, median, and other measures of central tendency, fail to do so \citep{satopaa2015combining}. To illustrate, consider two forecasters, each of whom reports a probability estimate for the occurrence of some future event, and suppose that they have access to different sets of information. More specifically, in reference to the diagram in Figure \ref{InfoStr}, 
...
forecaster 1 uses information in $U \cup M$ while forecaster 2 uses information in  $M \cup V$.
...
% 
 then clearly the combined information $U \cup M \cup V$ should give an aggregate forecast somewhat greater than $0.9$. In this simple scenario, however, measures of central tendency aggregate to $0.9$. Therefore they fail to account for the  information heterogeneity among the forecasters. 
% Instead, they assume that all forecasters use the same information. 
 This under-estimates the amount of combined information and produces aggregates that are under-confident and have poor out-of-sample performance  \citep{satopaa, satopaa2014probability}. 
...
In response to this need, for the past three years I have collaborated with the GJP and used their large forecasting database to test hypotheses and develop fundamental theory of an alternative source of forecast heterogeneity. In particular, my dissertation formalizes and proposes \textit{information diversity} as an alternative to measurement error.
...
%  
and apply the resulting model, named the \textit{Gaussian partial information model}, to real-world prediction polls, involving both probability and real-valued forecasts. In both cases, 
...
Even though our work corrects some of the common misconceptions and lays down foundations upon which more principled public reporting can be developed, much work remains to be done. One direction is 
...
to explore interactions between hospital and patient characteristics. For instance, 
