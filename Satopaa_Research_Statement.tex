...
%\subsection*{Motivation} \vspace{-0.2em}
 \vspace{0.5em}
\noindent
\textbf{2. MODELING, AGGREGATING, AND IMPROVING PREDICTIONS:}  Modern social and computer networks facilitate collection of multiple
...
%Past literature has distinguished two types of polling: prediction and opinion polling. In broad terms, an opinion poll is a survey of public opinion, whereas a prediction poll involves
  agents' predictions about the value of some quantity of interest \citep{goel2010prediction}. These so called \textit{prediction polls} may ask the agents, for instance,  who they think will win the next presidential election.
...
the forecasters use respective sets $U$ and $M$ that are disjoint subsets of all information about the future event. 
...
\textbf{2.2. CURRENT AND FUTURE RESEARCH:}
 Given that my dissertation proposes an alternative to a core statistical concept, namely measurement error, it 
...
This leads to a range of theoretical and applied projects in statistics and other related fields. 
...
explore interactions between hospital and patient characteristics. 
%For instance, 
...
Such a model would a) indicate for each hospital the type of patients it treats particularly well  
% what kind of hospitals are good at treating what kind of patients
  and b) form the basis of a new publicly available website where patients can type in their personal information and compare performance estimates that are personalized and hence more relevant to them. 
...
are facing in todayÕs Òbig dataÓ boom. All in all, however, I enjoy working with subject-matter experts, always
...
\marginpar{make sure all references look the same; some are misc and some are journals}
